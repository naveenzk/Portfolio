...
const projects = [
  {
    ...
    description: 'A comprehensive ERP system managing the full business lifecycle — from procurement and inventory to sales, production, and financial operations.',
    features: [
  ...
  },
  {
    ...
    description: 'End-to-end automation systems powered by AI agents and n8n workflows. Integrates with multiple business tools to automate repetitive processes and reporting.',
    features: [
...
export default function Projects() {
  ...
  return (
    <section id="projects" className="py-16 md:py-24 bg-[#080212] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6" ref={ref}>
        ...
        >
          <span className="text-purple-400 text-xs sm:text-sm font-semibold tracking-widest uppercase mb-3 block">What I've Built</span>
        ...
        <div className="grid lg:grid-cols-2 gap-5 sm:gap-6 mb-5 sm:mb-6">
          {projects.filter(p => p.featured).map((project, i) => (
            <motion.div
              ...
              className={`relative glass rounded-2xl p-5 sm:p-7 border ${project.border} transition-all duration-300 group overflow-hidden`}
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-start justify-between mb-5">
                  <div>
                    <span className={`inline-flex text-xs font-semibold px-2.5 py-1 rounded-full border ${project.badgeColor} mb-2.5`}>
                      {project.badge}
                    </span>
                    <div className="flex items-center gap-2.5">
                      <project.icon size={20} className="text-purple-300 flex-shrink-0" />
                      <h3 className="text-lg sm:text-xl font-bold text-white">{project.title}</h3>
                    </div>
                    <p className="text-slate-500 text-xs sm:text-sm mt-1">{project.subtitle}</p>
                  </div>
                ...
                <p className="text-slate-400 text-xs sm:text-sm leading-relaxed mb-5">{project.description}</p>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-1.5 gap-x-2 mb-5">
        ...
        <div className="grid sm:grid-cols-2 gap-5 sm:gap-6">
          {projects.filter(p => !p.featured).map((project, i) => (
            <motion.div
              ...
              className={`glass rounded-2xl p-5 sm:p-6 border ${project.border} transition-all duration-300 group`}
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    <project.icon size={17} className="text-slate-400 flex-shrink-0" />
